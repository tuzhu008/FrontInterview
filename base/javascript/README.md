...
* 消除代码裕兴的一些不安全之处，保证代码运行的安全
* 提高编译器效率，增加运行速度
* 严格模式禁用了在未来版本中可能会定定义的一些语法，为未来新版本的 Javascript 做好铺垫
...
* 
现在网站的 JS 都会进行压缩，一些文件用了严格模式，而另一些没有。这时，这些本来是严格模式的文件被 merge 后，这个串就到了文件中间，不仅没有指示严格模式，范围在压缩后浪费了字节。
